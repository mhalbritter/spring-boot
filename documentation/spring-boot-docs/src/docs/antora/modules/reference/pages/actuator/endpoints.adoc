...
`FRAMEWORK` enforces the limit from the outside without passing the duration to the indicator (thread interruption for blocking checks, javadoc:reactor.core.publisher.Mono#timeout(java.time.Duration)[] for reactive ones).
...
| `db`
| JDBC query timeout, in whole seconds rounded up.

| `elasticsearch`
| Connection request and response timeout of the cluster health request, in whole milliseconds.

| `hazelcast`
| Maximum lifespan of the transactional health check, in whole milliseconds.
