...
NOTE: The `hazelcast` indicator uses timeout mode `NATIVE`, so configprop:management.health.hazelcast.timeout[] and configprop:management.health.defaults.timeout[] bound the maximum lifespan of the transactional health check using javadoc:com.hazelcast.transaction.TransactionOptions#setTimeout(long,java.util.concurrent.TimeUnit)[setTimeout(long, TimeUnit)] on javadoc:com.hazelcast.transaction.TransactionOptions[].

NOTE: The `elasticsearch` indicator uses timeout mode `NATIVE`, so configprop:management.health.elasticsearch.timeout[] and configprop:management.health.defaults.timeout[] bound the cluster health request using per-request javadoc:org.apache.hc.client5.http.config.RequestConfig[] connection lease and response timeouts on javadoc:co.elastic.clients.transport.rest5_client.low_level.RequestOptions[].
