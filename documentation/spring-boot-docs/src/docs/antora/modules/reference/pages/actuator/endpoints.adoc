...
NOTE: The `cassandra` indicator uses timeout mode `NONE`, so configprop:management.health.cassandra.timeout[] and configprop:management.health.defaults.timeout[] do not bound how long that check can run.
It inspects javadoc:com.datastax.oss.driver.api.core.CqlSession#getMetadata()[driver metadata] (node states) only, with no CQL round-trip.
