...
| `jms`
| Watchdog which bounds how long `jakarta.jms.Connection#start()` may block, closing the connection when the limit is exceeded, with a granularity of one millisecond.
When no timeout is configured, a five second watchdog still guards hung calls.
