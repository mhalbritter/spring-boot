...
[[actuator.endpoints.health.timeouts]]
=== Health Indicator Timeouts

You can run health indicators with a maximum execution time.
You can set a default timeout for all indicators with configprop:management.health.defaults.timeout[].
To configure a timeout for a specific indicator, use `management.health.key.timeout`, where `key` is the indicator name (for example, `db`, `redis`, `cassandra`, etc.) from the xref:actuator/endpoints.adoc#actuator.endpoints.health.auto-configured-health-indicators[auto-configured indicators table] above.
For nested composite indicators, the `/` path separator in the indicator name is replaced with `.` in the property name (for example, a nested indicator named `datasource/primary` is configured with `management.health.datasource.primary.timeout`).
When a health check exceeds its timeout, that indicator reports status `DOWN` with a detail `reason: "timeout"`.
A timeout must be positive.


[[actuator.endpoints.health.timeouts.enforcement]]
==== How the Timeout Is Enforced

A configured timeout is always honored; how the limit is applied depends on the indicator's javadoc:org.springframework.boot.health.contributor.TimeoutEnforcement[].
`INDICATOR` passes the duration to the indicator, which honors it with stack-specific APIs, and runs the check on the thread which asked for the health.
An indicator declaring `INDICATOR` has to override `health(Duration)`, otherwise it reports `DOWN` when a timeout is configured.
`FRAMEWORK` enforces the limit from the outside (thread interruption for blocking checks, javadoc:reactor.core.publisher.Mono#timeout(java.time.Duration)[] for reactive ones) and runs the check on a thread of its own.
The duration is passed to such an indicator as well, so that a check which can bound part of its work does so and fails early, but the limit which is guaranteed is the one the framework applies.
An indicator which declares nothing uses `FRAMEWORK`.
Blocking `FRAMEWORK` checks run on a dedicated thread pool.
At most four `FRAMEWORK` executions of the same indicator run at the same time, which bounds the pool even when a check does not stop after its timeout has elapsed; further executions report `DOWN` with a detail `reason: "concurrency-limit"`.
Callers which see details and callers which do not are counted separately, so that a caller which is not authorized to see details cannot use up the executions of one which is.
While a `FRAMEWORK` execution is running, callers requesting the same indicator do not start their own; they attach to that running execution and share its result instead of counting against the limit again.

WARNING: Because probes share a check, a `FRAMEWORK` check must not depend on who asked for the health.
A shared check carries nothing of its caller: a reactive one runs with an empty Reactor context and a blocking one on a pool thread, so an indicator which reads the security context, a tenant, or anything else bound to the caller sees none of it.
The requested amount of detail is the exception: a separate check is started for the callers which see details and for those which do not.

WARNING: A health timeout bounds the reported result, not the work behind it.
It cannot stop a check which ignores interruption, e.g. blocking client stuck in a socket read: the response is `DOWN` after the configured duration, but the thread running that check stays occupied until the read returns, if it ever does.
Configure connect and read timeouts on the client itself (socket, driver, or connection pool), and treat the health timeout as an additional safety net rather than the primary limit.
Concurrent probes share one such stuck check, but a probe arriving after the timeout has elapsed starts a new one, so a hanging dependency parks up to eight threads per indicator, four for the callers which see details and four for those which do not, before further probes are rejected with `reason: "concurrency-limit"`.


[[actuator.endpoints.health.timeouts.auto-configured-indicators]]
==== Timeouts of the Auto-configured Indicators

The following auto-configured indicators enforce the timeout themselves (`INDICATOR`), by handing it to the client they use.
Every other indicator is bounded by the framework (`FRAMEWORK`).
An indicator may round the timeout up to the granularity of its client, so the effective limit is never shorter than the configured one:

[cols="2,6"]
|===
| Key | How the timeout is applied

| `elasticsearch`
| Cancellation of the cluster health request once the timeout has elapsed.

| `mongodb`
| Command execution time, in whole milliseconds.

| `neo4j`
| Timeout of the Cypher health check transaction, with a granularity of one millisecond.
|===

The `FRAMEWORK` indicators below block on a client of their own, which is where the real limit should be configured:

[cols="2,6"]
|===
| Key | Client settings which limit blocking time

| `db`
| Connection pool settings such as `spring.datasource.hikari.connection-timeout`, and the login and socket timeouts of the JDBC driver.
The health timeout is additionally applied as the JDBC query timeout, in whole seconds rounded up, which leaves acquiring the connection unbounded.

| `hazelcast`
| `hazelcast.client.invocation.timeout.seconds` for a client instance, `hazelcast.operation.call.timeout.millis` for a member.
The health timeout is additionally applied as the maximum lifespan of the health check transaction, which Hazelcast compares against on commit only.

| `jms`
| The connection and transport settings of your JMS `ConnectionFactory`.
The indicator additionally closes the connection if `jakarta.jms.Connection#start()` or `jakarta.jms.Connection#getMetaData()` has not returned within the health timeout, or within five seconds when no timeout is configured.

| `ldap`
| configprop:spring.ldap.base-environment[] (for example, `com.sun.jndi.ldap.connect.timeout` and `com.sun.jndi.ldap.read.timeout` entries for the JDK LDAP provider) or your LDAP `ContextSource` configuration.

| `mail`
| configprop:spring.mail.properties[] (for example, `mail.smtp.connectiontimeout` and `mail.smtp.timeout` for SMTP).

| `rabbit`
| configprop:spring.rabbitmq.connection-timeout[], configprop:spring.rabbitmq.channel-rpc-timeout[], and other client settings.

| `redis`
| configprop:spring.data.redis.connect-timeout[] and configprop:spring.data.redis.timeout[].
|===

The remaining indicators (`diskspace`, `ping`, `ssl`, `livenessstate`, and `readinessstate`) answer from local state and never wait on the network.

NOTE: The `cassandra` indicator inspects the driver metadata returned by `CqlSession#getMetadata()` only, with no CQL round-trip, and the `couchbase` indicator uses javadoc:com.couchbase.client.java.Cluster#diagnostics()[passive diagnostics] only, so neither waits on the network.
