...
You can run health indicators with a maximum execution time, if the health indicator supports it.
...
How the limit is applied depends on the indicator's timeout mode (see javadoc:org.springframework.boot.health.contributor.HealthIndicator#getTimeoutSupport[] and javadoc:org.springframework.boot.health.contributor.ReactiveHealthIndicator#getTimeoutSupport[]).
`NONE` ignores the configured timeout (Spring Boot logs a one-time warning per indicator name).
`NATIVE` lets the indicator honor the duration with stack-specific APIs.
`INTERRUPTION` enforces it from the outside (thread interruption for blocking checks, javadoc:reactor.core.publisher.Mono#timeout(java.time.Duration)[] for reactive ones).
`INTERRUPTION` is best-effort: blocking libraries do not always stop as soon as the timeout elapses, so wall-clock time can exceed the configured duration before `DOWN` is reported.
Blocking interruption-based checks use a bounded thread pool; if the pool rejects work, the check can fail with an error instead of a `DOWN` result with `reason: "timeout"`.

NOTE: With the `database` health indicator, JDBC timeouts are expressed in whole seconds; values under one second are rounded up to one second.

NOTE: The `redis` indicator uses timeout mode `INTERRUPTION` when you use javadoc:org.springframework.data.redis.connection.lettuce.LettuceConnectionFactory[].
With javadoc:org.springframework.data.redis.connection.jedis.JedisConnectionFactory[] (or any other javadoc:org.springframework.data.redis.connection.RedisConnectionFactory[]), the indicator uses timeout mode `NONE`, so those properties do not bound how long that check can run.
Use Lettuce if you want the Redis health check to participate in configured timeouts at all.

NOTE: The `mongo` indicator uses timeout mode `NATIVE`, so configprop:management.health.mongodb.timeout[] and configprop:management.health.defaults.timeout[] bound command execution time.
Values under one millisecond are rounded up to one millisecond.

NOTE: The `jms` indicator uses timeout mode `NATIVE`, so configprop:management.health.jms.timeout[] and configprop:management.health.defaults.timeout[] bound how long `jakarta.jms.Connection#start()` may block by closing the connection when that limit is exceeded.
When no timeout is configured, the indicator still uses a short internal watchdog for hung `jakarta.jms.Connection#start()` calls.

NOTE: The `hazelcast` indicator uses timeout mode `NATIVE`, so configprop:management.health.hazelcast.timeout[] and configprop:management.health.defaults.timeout[] bound the maximum lifespan of the transactional health check.

NOTE: The `elasticsearch` indicator uses timeout mode `NATIVE`, so configprop:management.health.elasticsearch.timeout[] and configprop:management.health.defaults.timeout[] bound the cluster health request.

NOTE: The `neo4j` indicator uses timeout mode `NATIVE`, so configprop:management.health.neo4j.timeout[] and configprop:management.health.defaults.timeout[] bound the Cypher health check.
Values under one millisecond are rounded up to one millisecond.

NOTE: The `rabbit` indicator uses timeout mode `NONE`, so configprop:management.health.rabbit.timeout[] and configprop:management.health.defaults.timeout[] do not bound how long that check can run.
Limit blocking time with configprop:spring.rabbitmq.connection-timeout[], configprop:spring.rabbitmq.channel-rpc-timeout[], and other client settings instead.

NOTE: The `ldap` indicator uses timeout mode `NONE`, so configprop:management.health.ldap.timeout[] and configprop:management.health.defaults.timeout[] do not bound how long that check can run.
Limit blocking time with configprop:spring.ldap.base-environment[] (for example, `com.sun.jndi.ldap.connect.timeout` and `com.sun.jndi.ldap.read.timeout` entries for the JDK LDAP provider) or your LDAP `ContextSource` configuration instead.

NOTE: The `mail` indicator uses timeout mode `NONE`, so configprop:management.health.mail.timeout[] and configprop:management.health.defaults.timeout[] do not bound how long that check can run.
Limit blocking time with configprop:spring.mail.properties[] (for example, `mail.smtp.connectiontimeout` and `mail.smtp.timeout` for SMTP) instead.

NOTE: The `cassandra` indicator uses timeout mode `NONE`, it inspects javadoc:com.datastax.oss.driver.api.core.CqlSession#getMetadata()[driver metadata] (node states) only, with no CQL round-trip.

NOTE: The `couchbase` indicator uses timeout mode `NONE`, it uses javadoc:com.couchbase.client.java.Cluster#diagnostics()[passive diagnostics] only.
