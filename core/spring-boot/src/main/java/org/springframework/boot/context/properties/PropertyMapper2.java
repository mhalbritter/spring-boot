 ...
 * you can use {@link Source#withNulls()}.
 *
 * @author Phillip Webb
 ...
 * @since 2.0.0
 */
public final class PropertyMapper2 {

	private static final PropertyMapper2 INSTANCE = new PropertyMapper2(null, null);

	private final @Nullable PropertyMapper2 parent;

	private final @Nullable SourceOperator sourceOperator;

	PropertyMapper2(@Nullable PropertyMapper2 parent, @Nullable SourceOperator sourceOperator) {
		this.parent = parent;
		this.sourceOperator = sourceOperator;
	}

	/**
	 * Return a new {@link PropertyMapper2} instance that applies the given
	 * {@link SourceOperator} to every source.
	 * @param operator the source operator to apply
	 * @return a new property mapper instance
	 */
	public PropertyMapper2 alwaysApplying(SourceOperator operator) {
		Assert.notNull(operator, "'operator' must not be null");
		return new PropertyMapper2(this, operator);
	}
	...
	private <T extends @Nullable Object> Source<T> getSource(Supplier<T> supplier) {
		...
		return new Source<>(SingletonSupplier.of(supplier), (ignored) -> true);
	}
	 ...
	 * @return the property mapper
	 */
	public static PropertyMapper2 get() {
		return INSTANCE;
	}
	...
	public static final class Source<T extends @Nullable Object> {
		 ...
		 */
		public <R extends @Nullable Number> Source<Integer> asInt(Function<@NonNull T, R> adapter) {
		 ...
		 * @return a new filtered source instance
		 */
		public Source<T> whenNot(Predicate<@NonNull T> predicate) {
			Assert.notNull(predicate, "'predicate' must not be null");
		 ...
		 * @return a new filtered source instance
		 */
		public Source<T> when(Predicate<@NonNull T> predicate) {
			Assert.notNull(predicate, "'predicate' must not be null");
			return new Source<>(this.supplier, this.predicate.and(predicate));
		}

		/**
		 * Return a version of this source that can be used to complete mappings with
		 * support {@code null} values.
		 * @return a new {@link WithNulls} instance
		 */
		public WithNulls<T> withNulls() {
			return new WithNulls<>(this);
		}
		...
		boolean test(T t) {
			Assert.state(t != null, "'t' must not be null");
		...
		public static class WithNulls<T extends @Nullable Object> {
			 ...
			 * @return a new mapped instance or the original instance
			 */
			public <I extends @Nullable Object> I to(I instance, Mapper<I, ? super T> mapper) {
				Assert.notNull(instance, "'instance' must not be null");
				Assert.notNull(mapper, "'mapper' must not be null");
				T value = getValue();
				if (value == null || test(value)) {
			...
			@FunctionalInterface
			public interface Mapper<I extends @Nullable Object, V> {

				I map(I instance, @Nullable V value);
