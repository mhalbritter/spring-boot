...
class MongoReactiveHealthIndicatorTests {
	...
	void mongoIsUp() {
		...
		ListDatabasesPublisher<Document> listPublisher = mockListDatabasesPublisher(
				Flux.just(new Document("name", "test"), new Document("name", "admin")));
	...
	void mongoUsesFirstDatabaseWhenAdminIsNotVisible() {
		MongoClient mongoClient = mock(MongoClient.class);
		ListDatabasesPublisher<Document> listPublisher = mockListDatabasesPublisher(
				Flux.just(new Document("name", "test")));
		given(mongoClient.listDatabases(Document.class)).willReturn(listPublisher);
	...
	void getTimeoutEnforcementIsIndicator() {
		MongoClient mongoClient = mock(MongoClient.class);
		assertThat(new MongoReactiveHealthIndicator(mongoClient).getTimeoutEnforcement())
