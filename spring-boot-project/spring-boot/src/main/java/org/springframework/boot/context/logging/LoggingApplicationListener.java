...
public class LoggingApplicationListener implements GenericApplicationListener {
	...
	private void onApplicationPreparedEvent(ApplicationPreparedEvent event) {
		...
		if (!beanFactory.containsBean(LOGGING_SYSTEM_BEAN_NAME)) {
			...
			beanFactory.registerSingleton(LOGGING_SYSTEM_BEAN_NAME, loggingSystem);
		}
	...
	protected void initialize(ConfigurableEnvironment environment, ClassLoader classLoader) {
		...
		LoggingSystem loggingSystem = this.loggingSystem;
		Assert.notNull(loggingSystem, "'loggingSystem' must not be null");
		initializeSystem(environment, loggingSystem, this.logFile);
		initializeFinalLoggingLevels(environment, loggingSystem);
		registerShutdownHookIfNecessary(environment, loggingSystem);
	}
