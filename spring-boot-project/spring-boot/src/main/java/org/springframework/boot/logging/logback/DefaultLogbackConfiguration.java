...
class DefaultLogbackConfiguration {
	...
	private void defaults(LogbackConfigurator config) {
		...
		config.conversionRule("correlation", CorrelationConverter.class);
		config.getContext()
			.putProperty("CONSOLE_LOG_PATTERN", resolve(config, "${CONSOLE_LOG_PATTERN:-"
					...
					+ "%clr(%correlation){}%clr(${PID:- }){magenta} %clr(---){faint} %clr([%15.15t]){faint} %clr(%-40.40logger{39}){cyan} "
		...
		config.getContext()
			.putProperty("FILE_LOG_PATTERN", resolve(config, "${FILE_LOG_PATTERN:-"
					+ "%d{${LOG_DATEFORMAT_PATTERN:-yyyy-MM-dd'T'HH:mm:ss.SSSXXX}} ${LOG_LEVEL_PATTERN:-%5p}%correlation{}${PID:- } --- [%t] "
