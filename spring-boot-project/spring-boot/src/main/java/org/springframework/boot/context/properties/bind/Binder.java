...
public class Binder {
	 ...
	 * @see #bind(ConfigurationPropertyName, Bindable, BindHandler)
	 */
	public <T> @Nullable T bindOrCreate(String name, Class<T> target) {
	 ...
	 */
	public <T> @Nullable T bindOrCreate(String name, Bindable<T> target) {
	 ...
	 * @see #bindOrCreate(ConfigurationPropertyName, Bindable, BindHandler)
	 */
	public <T> @Nullable T bindOrCreate(String name, Bindable<T> target, BindHandler handler) {
	 ...
	 * @since 2.2.0
	 */
	public <T> @Nullable T bindOrCreate(ConfigurationPropertyName name, Bindable<T> target, @Nullable BindHandler handler) {
		return bind(name, target, handler, true);
	}

	private <T> @Nullable T bind(ConfigurationPropertyName name, Bindable<T> target, @Nullable BindHandler handler, boolean create) {
	...
	private <T> @Nullable T bind(ConfigurationPropertyName name, Bindable<T> target, BindHandler handler, Context context,
	...
	private <T> @Nullable T handleBindResult(ConfigurationPropertyName name, Bindable<T> target, BindHandler handler,
		...
		if (result == null && create) {
			...
			if (result == null) {
				...
				List<DataObjectBinder> binders = this.dataObjectBinders.get(target.getBindMethod());
				Assert.notNull(binders, "'binders' must not be null");
				binders
	...
	private @Nullable<T> ConfigurationProperty findProperty(ConfigurationPropertyName name, Bindable<T> target,
			Context context) {
	...
	private @Nullable Object fromDataObjectBinders(@Nullable BindMethod bindMethod, Function<DataObjectBinder, Object> operation) {
		List<DataObjectBinder> binders = this.dataObjectBinders.get(bindMethod);
		Assert.notNull(binders, "'binders' must not be null");
		return binders
			.stream()
			.map(operation)
